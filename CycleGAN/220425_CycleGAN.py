...
EPOCHS = 100
version = 'v1'
version_dir = './weight_files/'+version
weight_name = '100_epochs'


### load dataset
# original_picture
ds_ori = tf.keras.utils.image_dataset_from_directory('../images/origin', batch_size=batch_size, image_size = orig_img_size)

# pixel_art
ds_tar = tf.keras.utils.image_dataset_from_directory('../images/target', batch_size=batch_size, image_size = orig_img_size)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, img in enumerate(ds_ori_t.take(self.num_img)):
            prediction = self.model.gen_G(img)[0].numpy()
            prediction = (prerdiction * 127.5 + 127.5).astype(np.uint8)
...
cycle_gan_model.fit(
    tf.data.Dataset.zip((ds_ori_t, ds_tar_t)),
    epochs=EPOCHS,
...
### save models
exist_dir = os.path.exists(version_dir)
...
cycle_gan_model.save_weights(version_dir+'/'+weight_name)
